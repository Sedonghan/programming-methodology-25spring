...
TEST(TodoListTest, RemoveInvalidIndexThrows) {
    ...
    EXPECT_THROW(todo.remove_task(-1), std::out_of_range);  // 음수 인덱스 제거 시도
    EXPECT_THROW(todo.remove_task(33), std::out_of_range);
...
TEST(TodoListTest, AddTooManyTasksThrows) {
    ...
    EXPECT_THROW(todo.add_task("Overflow Task"), std::overflow_error);  // 33번째 추가하면 터져야 한다
